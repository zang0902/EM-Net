    ...
    train_transform = transforms.Compose(
        [
            ...
            transforms.RandCropByPosNegLabeld(
                keys=["image", "label"],
                label_key="label",
                spatial_size=(args.roi_x, args.roi_y, int(args.roi_z/3)*2),
                pos=1,
                neg=1,
                # num_samples=4,
                num_samples=args.crop_nums,
...
def get_loader_msd_pancreas(args):
    data_dir = args.data_dir
    datalist_json = os.path.join(data_dir, args.json_list)
    train_transform = transforms.Compose(
        [
